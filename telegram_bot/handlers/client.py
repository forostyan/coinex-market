...
import psycopg2
from config_db import host, port, user, password, db_name
...
async def process_callback_wallet_btn(callback: types.CallbackQuery):
    ...
    await callback.message.answer('Ваш баланс:\n' +
                                  f'BTC: {data[0]}\n' +
                                  f'ETH: {data[1]}\n' +
                                  f'BNB: {data[2]}\n' +
                                  f'USDT: {data[3]}\n' +
...
async def process_callback_deposit_btn(callback: types.CallbackQuery):
    await callback.message.answer('Вот кошелек для перевода суммы: [кошельки пока не созданы]\n' +
                                  f'{callback.data}\n' +
...
async def process_callback_withdraw_coin_btn(callback: types.CallbackQuery, state: FSMContext):
...
async def process_callback_withdraw_address_btn(message: types.Message, state: FSMContext):
    async with state.proxy() as withdraw_data:
    ...
    with connection.cursor() as cursor:
        ...
        if cur_balance < float(withdraw_data['summ']):
            await message.answer(f'На вашем счету недостаточно средств',
                                 reply_markup=client_kb.back_kb)
        else:
            cursor.execute(f"""UPDATE
               public."user"
            ...
            connection.commit()
            cursor.execute(f"""INSERT INTO public.withdraw
            ...
            await message.answer(f'Введенная сумма будет переведена на выбранный адрес {withdraw_data["address"]}',
                                 reply_markup=client_kb.back_kb)
...
async def process_callback_create_deal_btn(callback: types.CallbackQuery):
    ...
    await callback.message.answer('Купить или продать криптовалюту?',
                                  reply_markup=client_kb.inline_create_deal_kb)
    await callback.message.delete()
    await callback.answer()
...
async def process_callback_create_buy_btn(callback: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        data['side'] = callback.data
    await FSMCreate.next()
    await callback.message.answer('Выберите криптовалюту',
                                  reply_markup=client_kb.coins_kb)
    await callback.message.delete()
    await callback.answer()
...
async def process_callback_create_coin_btn(callback: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        data['coin'] = callback.data
    await FSMCreate.next()
    await callback.message.answer('Выберите фиат',
                                  reply_markup=client_kb.inline_create_fiat_kb)
    await callback.message.delete()
    await callback.answer()
...
async def process_callback_create_fiat_btn(callback: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
...
async def process_callback_create_address_btn(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
...
async def process_callback_create_rate_btn(message: types.Message, state: FSMContext):
    ...
    with connection.cursor() as cursor:
        ...
        if cur_balance < float(data['max']) and data['side'] == 'sell':
            await message.answer(f'На вашем счету недостаточно средств для создания сделки',
                                 reply_markup=client_kb.back_kb)
        else:
            cursor.execute(f"""INSERT INTO public."order"
    ...
    async with state.proxy() as data:
        await message.answer(f'Создана сделка:\n {data["side"]}\n' +
...
async def process_callback_take_btn(callback: types.CallbackQuery):
    ...
    await callback.message.answer('Купить или продать криптовалюту?',
                                  reply_markup=client_kb.inline_take_kb)
    await callback.message.delete()
    await callback.answer()
...
async def process_callback_take_buy_btn(callback: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        data['side'] = callback.data
...
async def process_callback_take_coin_btn(callback: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        data['coin'] = callback.data
...
async def process_callback_my_deals_btn(callback: types.CallbackQuery):
    await callback.message.answer('Выберите тип сделок',
                                  reply_markup=client_kb.inline_deals_kb)
    await callback.message.delete()
    await callback.answer()
...
async def process_callback_my_closed_btn(callback: types.CallbackQuery):
    await callback.message.answer('Закрытых сделок пока нет(тк. взаимодействие пока недоступно)',
                                  reply_markup=client_kb.back_kb)
    await callback.message.delete()
    await callback.answer()
